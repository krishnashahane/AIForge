...
- [ ] Checksums (optional, for GitHub release assets):
  - `shasum /tmp/aiforge-<version>.tgz > /tmp/aiforge-<version>.tgz.sha1`
  - `shasum -a 256 /tmp/aiforge-<version>.tgz > /tmp/aiforge-<version>.tgz.sha256`

## 5) Publish to npmjs (aiforge)

- [ ] Confirm npm session / 2FA: `npm whoami`
- [ ] Publish: `npm publish`
- [ ] Verify: `npm view aiforge version`
...
- [ ] GitHub release (title `X.Y.Z`, body = changelog bullets for that version):
  - `gh release create vX.Y.Z --title X.Y.Z --notes-file /tmp/aiforge-release-notes.md`
  - Attach assets if you built them: `/tmp/aiforge-<version>.tgz*`

## 7) GitHub Packages (npm.pkg.github.com) (optional)

GitHub Packages for npm typically expects a **scoped** package name (e.g. `@steipete/aiforge`). If you want that in addition to `aiforge` on npmjs, publish it as a separate package name/version strategy.
