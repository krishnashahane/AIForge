...
describe("aiforge/node litellm", () => {
	...
	it("caches catalog under AIFORGE_CACHE_DIR", async () => {
		const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "aiforge-"));
		let calls = 0;
		...
		const env = { AIFORGE_CACHE_DIR: tmp } as Record<string, string | undefined>;
